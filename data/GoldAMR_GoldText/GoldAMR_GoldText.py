...
# Lint as: python3
'''The goldAMRgoldText dataset contains a colelction of human-made AMR graphs
paired with their equivalent sentences in English.
...
_DESCRIPTION = '''The goldAMRgoldText dataset contains a colelction of human-made AMR graphs
paired with their equivalent sentences in English.
...
class GoldAMR_GoldTextConfig(datasets.BuilderConfig):
    '''BuilderConfig for  goldAMRgoldText.'''
...
class GoldAMR_GoldText(datasets.GeneratorBasedBuilder):
    ''' goldAMRgoldText dataset.'''

    VERSION = datasets.Version('0.0.1')

    BUILDER_CONFIG_CLASS = GoldAMR_GoldTextConfig
    BUILDER_CONFIGS = [
        GoldAMR_GoldTextConfig(dataset, lang)
        for dataset in _DATASET_FILES.keys()
        for lang in _LANGUAGES
    ]
    BUILDER_CONFIGS += [GoldAMR_GoldTextConfig('all', lang) for lang in _LANGUAGES]
